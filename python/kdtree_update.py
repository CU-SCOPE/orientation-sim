...
class kdTree():
    ...
    def buildTree(self,level,points,index,parent):
        ...
        if not level:
            ...
            if index % 2:
                facesbin = self.nodes[parent].facesLeft
            else:
                facesbin = self.nodes[parent].facesRight
        ...
        if level < 5:
            pointsLeft = points[0:medInd,:]
            self.buildTree(level+1, pointsLeft, index*2+1, index)
            pointsRight = points[medInd+1:,:]
    ...
    def checkFaces(self, faces, point, clPoint, dist):
        ...
        for i in range(len(faces)):
            face[0,:] = faces[i].vert0
            face[1,:] = faces[i].vert1
            face[2,:] = faces[i].vert2
            tmpDist, tmpPoint = self.pointTriangleDistance(face, point)
            if tmpDist < dist:
                dist = tmpDist
                clPoint = tmpPoint
            return dist, clPoint
    ...
    def traverse(self, ind, point, clPoint, dist):
        ...
        if distPlane < dist:
            if(left):
                dist, clPoint = self.checkFaces(current.facesRight, point, clPoint, dist)
            else:
                dist, clPoint = self.checkFaces(current.facesLeft, point, clPoint, dist)
